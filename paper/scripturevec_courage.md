...
The rationales locate the mechanism, and rule out the flatter reading on which the vector merely supplies a scriptural gloss for a verdict the model had already settled. Steered on ignatian, the model's adherence to Scripture is itself intensified — it invokes Scripture and the Fathers more heavily, in dose with the steering (Table 3) — and that intensified adherence is what raises its confidence in the option Scripture has been made to endorse. On ignatian that option is the cowardly one, so deeper adherence is deeper conviction in the wrong act; the discrete choice moves little because the model was already leaning that way, but the margin, and the rationale, register the hardening. The vector strengthens the model's hold on Scripture, and where the model lacks the practical wisdom to tell Scripture rightly used from Scripture bent to a false end, strengthening that hold strengthens the error. What it supplies is the appetite for the good; what it cannot supply is the judgment that would keep the appetite from fastening on a counterfeit.
...
**Translation dependence.** The corpora are King James. The Wikipedia control shows the effect is specific to Scripture rather than to authoritative prose in general, but it does not separate the content of Scripture from the archaic, authoritative register of this particular translation, and a different translation could yield a different geometry. A modern translation, and a secular but archaic and authoritative corpus, run through the same pipeline, would draw that line, and we take it to be the most important control this study leaves for the next.
...
**The margin is a proxy.** The courage margin is taken from a single forward pass in answer-only mode. It agrees with the model's discrete choice on control, but it does not capture the extended reasoning a model produces when it deliberates at length, and the two can diverge.
